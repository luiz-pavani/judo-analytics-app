...
// --- ESTILOS & COMPONENTES AUXILIARES (Globais para evitar erro de escopo) ---
...
export default function JudoPlayer() {
  ...
  async function fetchData() {
    ...
    if (dbEvents) {
       const formattedEvents = dbEvents.map(e => ({ ...e, videoId: e.video_id, corTecnica: e.cor_tecnica, grupo: e.grupo }));
    ...
    if (dbMeta) {
       ...
       dbMeta.forEach((m: any) => {
          map[m.video_id] = { eventName: m.event_name, date: m.date, category: m.category, phase: m.phase, location: m.location, whiteId: m.white_id, blueId: m.blue_id };
       });
       setMetadataMap(map);
    }
  }
  ...
  function exportarBackup() { const backupData = { version: "28.21", date: new Date().toISOString(), eventos, athletes, metadataMap, playlist }; const blob = new Blob([JSON.stringify(backupData, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `SMAARTPRO_BACKUP_${new Date().toLocaleDateString().replace(/\//g,'-')}.json`; document.body.appendChild(a); a.click(); document.body.removeChild(a); }
