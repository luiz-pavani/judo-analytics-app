...
// ============================================================================
// 1. CONFIGURAÇÃO VISUAL (THEME SYSTEM)
// ============================================================================
const THEME = {
  bg: '#0f172a',        // Fundo Principal (Slate 900)
  card: '#1e293b',      // Cartões (Slate 800)
  cardBorder: '#334155',// Bordas (Slate 700)
  text: '#f8fafc',      // Texto Principal (Slate 50)
  textDim: '#94a3b8',   // Texto Secundário (Slate 400)
  ...
  surface: '#020617',   // Fundo Profundo (Inputs)
};

// ============================================================================
// 2. BANCO DE DADOS (JUDO KNOWLEDGE)
// ============================================================================
const DB_SHIDOS = [
  "Passividade", 
  "Falso Ataque", 
...
const DB_GOLPES: Record<string, string> = {
  ...
  "tai-otoshi": "TE-WAZA", "kata-guruma": "TE-WAZA", "sukui-nage": "TE-WAZA", 
  "obi-otoshi": "TE-WAZA", "uki-otoshi": "TE-WAZA", "sumi-otoshi": "TE-WAZA", 
  ...
  "uki-goshi": "KOSHI-WAZA", "ō-goshi": "KOSHI-WAZA", "koshi-guruma": "KOSHI-WAZA", 
  ...
  "ō-soto-gari": "ASHI-WAZA", "ō-uchi-gari": "ASHI-WAZA", "ko-soto-gari": "ASHI-WAZA", 
  ...
  "ō-guruma": "ASHI-WAZA", "ō-soto-guruma": "ASHI-WAZA", "ō-soto-otoshi": "ASHI-WAZA", 
  "tsubame-gaeshi": "ASHI-WAZA", "ō-soto-gaeshi": "ASHI-WAZA", "ō-uchi-gaeshi": "ASHI-WAZA", 
  ...
  "tawara-gaeshi": "SUTEMI-WAZA", "ura-nage": "SUTEMI-WAZA", "yoko-otoshi": "SUTEMI-WAZA", 
  "tani-otoshi": "SUTEMI-WAZA", "hane-makikomi": "SUTEMI-WAZA", "soto-makikomi": "SUTEMI-WAZA", 
  "uchi-makikomi": "SUTEMI-WAZA", "uki-waza": "SUTEMI-WAZA", "yoko-wakare": "SUTEMI-WAZA", 
  "yoko-guruma": "SUTEMI-WAZA", "yoko-gake": "SUTEMI-WAZA", "daki-wakare": "SUTEMI-WAZA", 
  "ō-soto-makikomi": "SUTEMI-WAZA", "uchi-mata-makikomi": "SUTEMI-WAZA", "harai-makikomi": "SUTEMI-WAZA", 
  "ko-uchi-makikomi": "SUTEMI-WAZA", "kani-basami": "SUTEMI-WAZA", "kawazu-gake": "SUTEMI-WAZA",

  // KATAME-WAZA (Técnicas de Solo)
  "kesa-gatame": "OSAEKOMI-WAZA", "kuzure-kesa-gatame": "OSAEKOMI-WAZA", "ushiro-kesa-gatame": "OSAEKOMI-WAZA", 
  "kata-gatame": "OSAEKOMI-WAZA", "kami-shihō-gatame": "OSAEKOMI-WAZA", "kuzure-kami-shihō-gatame": "OSAEKOMI-WAZA", 
  "yoko-shihō-gatame": "OSAEKOMI-WAZA", "tate-shihō-gatame": "OSAEKOMI-WAZA", "uki-gatame": "OSAEKOMI-WAZA", 
  "ura-gatame": "OSAEKOMI-WAZA",
  "nami-jūji-jime": "SHIME-WAZA", "gyaku-jūji-jime": "SHIME-WAZA", "kata-jūji-jime": "SHIME-WAZA", 
  "hadaka-jime": "SHIME-WAZA", "okuri-eri-jime": "SHIME-WAZA", "kataha-jime": "SHIME-WAZA", 
  "katate-jime": "SHIME-WAZA", "ryōte-jime": "SHIME-WAZA", "sode-guruma-jime": "SHIME-WAZA", 
  "tsukkomi-jime": "SHIME-WAZA", "sankaku-jime": "SHIME-WAZA", "dō-jime": "SHIME-WAZA",
  "ude-garami": "KANSETSU-WAZA", "ude-hishigi-jūji-gatame": "KANSETSU-WAZA", "ude-hishigi-ude-gatame": "KANSETSU-WAZA", 
...
export default function JudoPlayer() {
  ...
  // --- STATES DE VÍDEO & PLAYLIST ---
  const [playlist, setPlaylist] = useState<PlaylistItem[]>([
    { id: 'kU_gjfnyu6A', type: 'YOUTUBE', name: 'Final Paris 2025' }
  ]);
  const [currentVideoIndex, setCurrentVideoIndex] = useState(0);
  const [showPlaylist, setShowPlaylist] = useState(false);
  
  // Vídeo Atual (Calculado)
  ...
  // --- STATES DE ANALYTICS (Contexto de Marcação) ---
  ...
  const [eventos, setEventos] = useState(() => {
    try {
      const salvos = localStorage.getItem('smaartpro_db_v9_expanded');
  ...
  // Salva automaticamente sempre que mudar 'eventos'
  useEffect(() => { 
    localStorage.setItem('smaartpro_db_v9_expanded', JSON.stringify(eventos)); 
  }, [eventos]);

  // --- EFEITOS DE SISTEMA (Resize & Teclado) ---
  useEffect(() => {
  ...
  const handleFileSelect = (e: any) => {
    ...
    // Se a playlist só tem o vídeo demo, substitui. Senão, adiciona.
  ...
  const proximoVideo = () => { 
    if (currentVideoIndex < playlist.length - 1) selecionarVideo(currentVideoIndex + 1); 
  };

  const videoAnterior = () => { 
    if (currentVideoIndex > 0) selecionarVideo(currentVideoIndex - 1); 
  };

  const removerDaPlaylist = (index: number, e: any) => { 
    e.stopPropagation(); 
    const novaLista = playlist.filter((_,i) => i !== index); 
    if (novaLista.length > 0) setPlaylist(novaLista); 
    
    if (index === currentVideoIndex && currentVideoIndex > 0) {
      setCurrentVideoIndex(currentVideoIndex - 1);
    } else if (index === currentVideoIndex && currentVideoIndex === 0 && novaLista.length > 0) {
      setCurrentVideoIndex(0);
    }
  };

  // ==========================================================================
  // 5. LÓGICA DO PLAYER (UNIFICADO)
  // ==========================================================================
  ...
  const onStateChange = (e: any) => { 
    ...
    if (e.data === 0) proximoVideo(); // Auto-advance
  };

  const onFileEnded = () => proximoVideo();

  // Loop de Sincronização de Tempo
  useEffect(() => {
  ...
  const irPara = (t: number) => { 
    const seekTime = Math.max(0, t - 2); // Rebobina 2s
    if (currentVideo.type === 'YOUTUBE' && youtubePlayerRef.current) { 
  ...
  // ==========================================================================
  // 6. LÓGICA DE REGISTRO (ARBITRAGEM & TÉCNICA)
  // ==========================================================================
  ...
  // ==========================================================================
  // 7. CÁLCULOS E ANALYTICS (NATIVO - SEM BIBLIOTECAS)
  // ==========================================================================

  // Auto-complete do Input
  useEffect(() => {
  ...
  const tempoDeLuta = useMemo(() => {
    ...
    for (const ev of evs) {
      if (ev.categoria === 'FLUXO') {
        if (ev.tipo === 'GOLDEN SCORE') isGS = true;
        if (ev.tipo === 'HAJIME') lastHajime = ev.tempo;
        if ((ev.tipo === 'MATE' || ev.tipo === 'SOREMADE') && lastHajime !== null) { 
          t += (ev.tempo - lastHajime); 
          lastHajime = null; 
        }
      }
    }
    if (lastHajime !== null && currentTime > lastHajime) t += (currentTime - lastHajime);
    return { total: t, isGS: isGS };
  ...
  const stats = useMemo(() => {
    ...
    const groupData = Object.keys(groups).map(g => ({ 
      name: g, 
      val: groups[g], 
      pct: (groups[g]/total)*100, 
      color: CORES_GRUPOS[g] 
    })).sort((a,b)=>b.val-a.val);
    
    // Volume e Eficiência
    const vol = { 
      branco: evs.filter((e:any)=>e.atleta==='BRANCO').length, 
      azul: evs.filter((e:any)=>e.atleta==='AZUL').length 
    };
    const eff = {
  ...
  const baixarCSV = () => {
    ...
    eventos.forEach((ev:any) => { 
      if(!evsByVid[ev.videoId]) evsByVid[ev.videoId]=[]; 
      evsByVid[ev.videoId].push(ev); 
    });
    ...
    link.download = `smaartpro_v9_${new Date().toISOString().slice(0,10)}.csv`; 
    link.click();
  };

  const getCorBorda = (ev: any) => { 
    if (ev.categoria === 'FLUXO') return THEME.cardBorder; 
    if (ev.atleta === 'AZUL') return THEME.primary; 
    return '#ffffff'; 
  };

  // --- COMPONENTE VISUAL: DONUT CHART (SVG) ---
  const SimpleDonut = ({ data }: { data: any[] }) => {
    let cumPct = 0;
    // Se não tiver dados, mostra cinza
    if(data.length === 0) return <div style={{width:'100px', height:'100px', borderRadius:'50%', border:`4px solid ${THEME.cardBorder}`, display:'flex', alignItems:'center', justifyContent:'center'}}><PieChart size={20} color={THEME.cardBorder}/></div>;

    const gradient = `conic-gradient(${data.map(d => { 
      const str = `${d.color} ${cumPct}% ${cumPct + d.pct}%`; 
      cumPct += d.pct; 
      return str; 
    }).join(', ')})`;
    ...
    <div style={{ maxWidth: '100%', minHeight: '100vh', margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: THEME.text, backgroundColor: THEME.bg, padding: '20px', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
        <h1 style={{ margin: 0, fontSize: isMobile?'22px':'26px', fontWeight: '800', letterSpacing: '-0.5px', display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '11px', color: THEME.textDim, marginLeft: '12px', background: THEME.card, padding: '2px 6px', borderRadius: '4px', border:`1px solid ${THEME.cardBorder}` }}>v9.1</span>
        </h1>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(2, 6, 23, 0.7)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ ...cardStyle, width: '100%', maxWidth: '480px', padding: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px'}}>
              <h2 style={{margin:0, color: THEME.primary, fontSize:'20px', display:'flex', alignItems:'center', gap:'10px', fontWeight:'700'}}><MousePointerClick size={24}/> REGISTRAR</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
              <div>
                <div style={{fontSize:'11px', color: THEME.textDim, marginBottom:'8px', fontWeight:'600'}}>QUEM?</div>
              ...
              <div>
                <div style={{fontSize:'11px', color: THEME.textDim, marginBottom:'8px', fontWeight:'600'}}>LADO?</div>
                <div style={{display:'flex', borderRadius:'8px', overflow:'hidden', border:`1px solid ${THEME.cardBorder}`}}>
            ...
            <div style={{marginBottom:'24px', position:'relative'}}>
              ...
              <div style={{position:'relative'}}>
                ...
                <input ref={inputRef} type="text" placeholder="Digite o nome..." value={modalNome} onChange={e=>setModalNome(e.target.value)} style={{width:'100%', padding:'12px 12px 12px 42px', background: THEME.surface, border:`1px solid ${THEME.cardBorder}`, color: THEME.text, borderRadius:'8px', fontSize:'15px', outline:'none', boxSizing:'border-box'}}/>
              </div>
              {sugestoes.length > 0 && (
                <div style={{position:'absolute', top:'100%', width:'100%', background: THEME.card, zIndex:100, border:`1px solid ${THEME.cardBorder}`, borderRadius:'8px', marginTop:'4px', boxShadow:'0 10px 15px -3px rgba(0, 0, 0, 0.3)', overflow:'hidden'}}>
                  {sugestoes.map(s=>(
                    <div key={s} onClick={()=>{setModalNome(s); const exact=Object.keys(DB_GOLPES).find(k=>k.toLowerCase()===s.toLowerCase()); if(exact) setModalGrupo(DB_GOLPES[exact] as any); setSugestoes([])}} style={{padding:'12px', borderBottom:`1px solid ${THEME.cardBorder}`, cursor:'pointer', display:'flex', justifyContent:'space-between', fontSize:'14px'}}>
            ...
            {/* Linha 3: Resultado */}
            <div>
              <div style={{fontSize:'11px', color: THEME.textDim, marginBottom:'8px', fontWeight:'600'}}>RESULTADO</div>
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        {/* COLUNA ESQUERDA (PLAYER & CONTROLES) */}
        <div style={{ flex: 3, width: '100%' }}>
          ...
          <div style={{...cardStyle, padding:'16px'}}>
             <div style={{fontSize:'11px', color: THEME.textDim, marginBottom:'10px', fontWeight:'600'}}>REGISTRO DE PUNIÇÕES</div>
        ...
        <div style={{ flex: 2, width: '100%', display:'flex', flexDirection:'column', gap:'20px' }}>
          ...
          {/* DASHBOARD DE ANALYTICS (NATIVO - SEM BIBLIOTECA) */}
          <div style={{ ...cardStyle, padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:'6px'}}>
              <div style={{display:'flex', justifyContent:'space-between', fontSize:'11px', color: THEME.textDim}}><span>⚪ Ataques: {stats.vol.branco}</span><span>🔵 Ataques: {stats.vol.azul}</span></div>
              <div style={{display:'flex', height:'8px', borderRadius:'4px', overflow:'hidden', background: THEME.surface}}>
                <div style={{width: `${(stats.vol.branco / (stats.vol.branco + stats.vol.azul || 1)) * 100}%`, background: 'white'}}></div>
            ...
            <div style={{display:'flex', gap:'20px', alignItems:'center'}}>
              ...
              <div style={{flex:1, display:'flex', flexDirection:'column', gap:'4px'}}>
                {stats.groupData.slice(0,4).map((g:any) => (
                  <div key={g.name} style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'11px'}}>
                    ...
                    <span style={{color:THEME.text, flex:1}}>{g.name}</span>
                    <span style={{color:THEME.textDim}}>{g.val}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {eventos.filter(e => e.videoId === currentVideo.name).map((ev: any) => (
                <div key={ev.id} style={{ 
                  ...
                  <div onClick={() => irPara(ev.tempo)} style={{cursor:'pointer', flex:1}}>
                    ...
                    <div style={{fontWeight:'600', color: ev.atleta === 'AZUL' ? THEME.primary : 'white', fontSize: '14px'}}>{ev.especifico || ev.tipo}</div>
