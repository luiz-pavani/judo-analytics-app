...
import { 
  ...
  LayoutDashboard, FolderOpen, TrendingUp // Novos ícones v28
} from 'lucide-react';
...
type Mode = 'PLAYER' | 'META'; // NOVO v28
...
export default function JudoPlayer() {
  ...
  const metaInputRef = useRef<any>(null); // NOVO v28
  const inputRef = useRef<any>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);

  // --- GLOBAL STATE ---
  const [mode, setMode] = useState<Mode>('PLAYER'); // NOVO v28
  ...
  const [metaFiles, setMetaFiles] = useState<any[]>([]); // Armazena os JSONs carregados para Meta
  const [targetAthleteId, setTargetAthleteId] = useState<string>(''); // Atleta foco da Meta
  ...
  // ==================================================================================
  // 3. CALCULOS (META MODE - v28.0)
  // ==================================================================================
  
  // Lista de atletas detectados nos arquivos carregados
  const metaDetectedAthletes = useMemo(() => {
      const detected = new Set<string>();
      metaFiles.forEach(f => {
          if (f.metadata) {
              const meta = f.metadata[Object.keys(f.metadata)[0]]; // Pega metadata do primeiro video do arquivo
  ...
  // Agregação de estatísticas para o Atleta Alvo
  const metaStats = useMemo(() => {
      if (!targetAthleteId || metaFiles.length === 0) return null;

      let totalMatches = 0;
      let totalWins = 0; // Simplificação: vamos contar Ippons como proxy de vitoria se não tiver resultado final
      let totalIppons = 0;
      let totalWazas = 0;
      let totalShidos = 0;
      let totalAttacks = 0;
      ...
      metaFiles.forEach(file => {
          totalMatches++;
          // Para cada arquivo, iterar sobre os eventos
          const fileEvents = file.eventos || [];
          const fileMeta = file.metadata ? Object.values(file.metadata)[0] as VideoMetadata : null;
          
          // Determinar a cor do atleta neste arquivo
          let myColor = '';
          if (fileMeta?.whiteId === targetAthleteId) myColor = 'BRANCO';
          else if (fileMeta?.blueId === targetAthleteId) myColor = 'AZUL';
          ...
          if (myColor) {
              fileEvents.forEach((ev: any) => {
                  if (ev.atleta === myColor) {
                      ...
                      if (ev.categoria === 'NE-WAZA' && ev.resultado && (ev.resultado.includes('Ippon') || ev.resultado.includes('ippon'))) {
                           totalIppons++; totalWins++;
                      }
      ...
      const radar = {
          FRENTE: directions.FRENTE / totalDir,
          TRAS: directions.TRAS / totalDir,
          ESQUERDA: directions.ESQUERDA / totalDir,
          DIREITA: directions.DIREITA / totalDir
      };
  ...
  const baixarCSV = () => { 
      ...
      const link = document.createElement("a"); link.href = encodeURI(csv); link.download = `smaartpro_v28_${new Date().toISOString().slice(0,10)}.csv`; link.click(); 
  };
  const exportarBackup = () => { const dadosBackup = { version: '28.0', timestamp: new Date().toISOString(), playlist, eventos, metadata: metadataMap, athletes }; const blob = new Blob([JSON.stringify(dadosBackup, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `smaartpro_${new Date().toISOString().slice(0, 10)}.json`; document.body.appendChild(link); link.click(); document.body.removeChild(link); };
  const importarBackup = (e: any) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (event) => { try { const json = JSON.parse(event.target?.result as string); if (json.eventos && Array.isArray(json.eventos)) { if (confirm("ATENÇÃO: Isso substituirá todos os dados atuais. Deseja continuar?")) { setEventos(json.eventos); if (json.playlist) { setPlaylist(json.playlist); setCurrentVideoIndex(0); } if(json.metadata) setMetadataMap(json.metadata); if(json.athletes) setAthletes(json.athletes); alert("Backup restaurado! 🥋"); } } else alert("Arquivo inválido."); } catch (err) { alert("Erro ao ler JSON."); } }; reader.readAsText(file); e.target.value = null; };
    ...
    <div ref={mainContainerRef} tabIndex={0} style={{ maxWidth: '100%', minHeight: '100vh', margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: THEME.text, backgroundColor: THEME.bg, padding: '20px', boxSizing: 'border-box', outline: 'none' }}>
      ...
      <div className="no-print" style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
        <h1 style={{ margin: 0, fontSize: isMobile?'24px':'32px', fontWeight: '800', letterSpacing: '-1px', display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '10px', color: THEME.text, marginLeft: '12px', background: THEME.cardBorder, padding: '4px 8px', borderRadius: '20px', border:`1px solid rgba(255,255,255,0.1)` }}>v28.0</span>
        </h1>
        <div style={{display:'flex', gap:'12px', alignItems:'center'}}>
          ...
          <div style={{display:'flex', gap:'8px'}}>
             <button onClick={() => setModalAthletes(true)} className="glow-hover" style={{...btnStyle, background: THEME.card, border: `1px solid ${THEME.cardBorder}`, color: THEME.text, padding:'10px', borderRadius:'10px'}} title="Atletas"><Users size={18}/></button>
             <button onClick={openMetadataModal} className="glow-hover" style={{...btnStyle, background: THEME.card, border: `1px solid ${THEME.cardBorder}`, color: '#f59e0b', padding:'10px', borderRadius:'10px'}} title="Contexto"><Trophy size={18}/></button>
             <button onClick={() => setModalHelp(true)} className="glow-hover" style={{...btnStyle, background: THEME.card, border: `1px solid ${THEME.cardBorder}`, color: THEME.textDim, padding:'10px', borderRadius:'10px'}} title="Ajuda"><Keyboard size={18}/></button>
          </div>
          <div style={{display:'flex', gap:'8px'}}>
             <button onClick={exportarBackup} style={{...btnStyle, background: THEME.card, border: `1px solid ${THEME.cardBorder}`, color: THEME.success, padding:'10px', borderRadius:'10px'}} title="Salvar"><Save size={18}/></button>
             <button onClick={() => backupInputRef.current.click()} style={{...btnStyle, background: THEME.card, border: `1px solid ${THEME.cardBorder}`, color: THEME.warning, padding:'10px', borderRadius:'10px'}} title="Abrir"><UploadCloud size={18}/></button>
                       ...
                       {metaStats ? (
                           ...
                           <div style={{display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:'15px', marginBottom:'30px'}}>
                               ...
                               <div style={{background: THEME.bg, padding:'20px', borderRadius:'12px', border:`1px solid ${THEME.cardBorder}`}}>
                                   ...
                                   <div style={{fontSize:'32px', fontWeight:'800', color:THEME.primary}}>{((metaStats.totalWins / metaStats.totalMatches)*100).toFixed(0)}%</div>
                               </div>
                               <div style={{background: THEME.bg, padding:'20px', borderRadius:'12px', border:`1px solid ${THEME.cardBorder}`}}>
                               ...
                               {metaStats.topTechs.map(([tech, count]: any) => (
                                   <div key={tech} style={{display:'flex', alignItems:'center', marginBottom:'10px', fontSize:'13px'}}>
                                       ...
                                       <div style={{flex:1, height:'8px', background:THEME.bg, borderRadius:'4px', overflow:'hidden'}}>
                                           <div style={{width:`${(count / metaStats.totalAttacks)*100}%`, height:'100%', background:THEME.success}}></div>
                                       </div>
            ...
            <div style={{ flex: 3, width: '100%', display:'flex', flexDirection:'column', gap:'20px' }}>
              ...
              <div style={{display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '16px'}}>
                <button onClick={() => iniciarRegistroRapido()} style={{...btnStyle, flexDirection:'column', gap:'12px', padding:'24px', background: THEME.primaryGradient, color:'white', fontSize:'20px', borderRadius:'16px', boxShadow: `0 10px 25px -5px ${THEME.primary}66`, border:`1px solid rgba(255,255,255,0.1)`}}>
                  <Tornado size={32} strokeWidth={1.5}/> <span style={{fontWeight:'800', letterSpacing:'1px'}}>NAGE-WAZA</span>
                </button>
            ...
            <div style={{ flex: 2, width: '100%', display:'flex', flexDirection:'column', gap:'20px' }}>
              ...
              <div style={{ flex: 1, display:'flex', flexDirection:'column' }}>
                <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginBottom:'12px' }}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    <h3 style={{margin:0, fontSize:'14px', color: 'white', fontWeight:'700', display:'flex', gap:'8px', alignItems:'center'}}><List size={18} color={THEME.primary}/> TIMELINE</h3>
      ...
      {modalIA && (
        <div className="no-print" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(2, 6, 23, 0.95)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ ...cardStyle, width: '100%', maxWidth: '600px', padding: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.8)', border: `1px solid ${THEME.cardBorder}` }}>
            ...
            <div style={{background: THEME.bg, padding:'15px', borderRadius:'8px', border:`1px solid ${THEME.cardBorder}`, color: THEME.textDim, fontSize:'12px', fontFamily:'monospace', whiteSpace:'pre-wrap', maxHeight:'300px', overflowY:'auto', marginBottom:'20px'}}>{generatedPrompt}</div>
