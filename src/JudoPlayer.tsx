...
import { Download, Trash2, PlayCircle, PauseCircle, Timer, Flag, Gavel, X, Search, CheckCircle, Link, Upload, Film, Youtube, MousePointerClick, Rewind, Gauge, List, SkipForward, SkipBack, Tv, BarChart2, Activity } from 'lucide-react';

// --- THEME SYSTEM ---
const THEME = {
  bg: '#0f172a', card: '#1e293b', cardBorder: '#334155', text: '#f8fafc', textDim: '#94a3b8',
  primary: '#3b82f6', primaryHover: '#2563eb', danger: '#ef4444', success: '#10b981', warning: '#eab308', surface: '#020617'
};
...
export default function JudoPlayer() {
  ...
  const [eventos, setEventos] = useState(() => {
    try { return JSON.parse(localStorage.getItem('smaartpro_db_v7') || '[]'); } catch { return []; }
  });
  useEffect(() => { localStorage.setItem('smaartpro_db_v7', JSON.stringify(eventos)); }, [eventos]);
  ...
  const adicionarYoutube = () => {
    const link = prompt("Link do YouTube:");
    if (link) {
  ...
  const removerDaPlaylist = (index: number, e: any) => {
    ...
    if (index >= novaLista.length) setCurrentVideoIndex(novaLista.length - 1);
  };
  ...
  const iniciarRegistroRapido = () => {
    ...
    setTempoCapturado(tempoExato); setModalAberto(true); setModalNome(''); setTimeout(() => inputRef.current?.focus(), 100);
  };

  const confirmarEContinuar = (resultado: string) => {
    const novoEvento = { id: Date.now(), videoId: currentVideo.name, tempo: tempoCapturado, categoria: 'TECNICA', grupo: modalGrupo, especifico: modalNome || "Técnica Geral", atleta: modalAtleta, lado: modalLado, corTecnica: CORES_GRUPOS[modalGrupo], resultado: resultado };
    setEventos([novoEvento, ...eventos]); setModalAberto(false);
  ...
  const analyticsData = useMemo(() => {
    ...
    evs.forEach((ev:any) => { counts[ev.grupo] = (counts[ev.grupo] || 0) + 1; });
    return Object.keys(counts).map(key => ({ name: key, value: counts[key], color: CORES_GRUPOS[key] || '#999' }));
  }, [eventos, currentVideo.name]);

  const volumeAtaque = useMemo(() => {
    const evs = eventos.filter((ev:any) => ev.videoId === currentVideo.name && ev.categoria === 'TECNICA');
    return {
    ...
    <div style={{ maxWidth: '100%', minHeight: '100vh', margin: '0 auto', fontFamily: 'Inter, system-ui, sans-serif', color: THEME.text, backgroundColor: THEME.bg, padding: '20px', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
        <h1 style={{ margin: 0, fontSize: isMobile?'22px':'26px', fontWeight: '800', letterSpacing: '-0.5px', display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '11px', color: THEME.textDim, marginLeft: '12px', background: THEME.card, padding: '2px 6px', borderRadius: '4px', border:`1px solid ${THEME.cardBorder}` }}>v7.0</span>
        </h1>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(2, 6, 23, 0.7)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ ...cardStyle, width: '100%', maxWidth: '480px', padding: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
            ...
            <div>
              ...
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}><button onClick={() => confirmarEContinuar('NADA')} style={{...btnStyle, padding: '14px', background: THEME.cardBorder, color: THEME.textDim}}>NADA</button><button onClick={() => confirmarEContinuar('YUKO')} style={{...btnStyle, padding: '14px', background: THEME.surface, border:`1px solid ${THEME.cardBorder}`, color: THEME.text}}>YUKO</button><button onClick={() => confirmarEContinuar('WAZA-ARI')} style={{...btnStyle, padding: '14px', background: `rgba(234, 179, 8, 0.1)`, border:`1px solid ${THEME.warning}`, color: THEME.warning}}>WAZA-ARI</button><button onClick={() => confirmarEContinuar('IPPON')} style={{...btnStyle, padding: '14px', background: THEME.text, color: '#000', border:`2px solid ${THEME.text}`}}>IPPON</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        <div style={{ flex: 3, width: '100%' }}>
          ...
          <div style={{...cardStyle, padding:'8px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'20px'}}>
             <div style={{display:'flex', alignItems:'center', gap:'16px'}}>
               <div style={{display:'flex', gap:'4px'}}>
                 <button onClick={videoAnterior} disabled={currentVideoIndex===0} style={{...btnStyle, background:'transparent', color:currentVideoIndex===0?THEME.cardBorder:THEME.text}}><SkipBack size={18}/></button>
                 <button onClick={togglePlay} style={{...btnStyle, background:'transparent', color:THEME.primary}}>{isPlaying ? <PauseCircle size={32} fill={THEME.primary} color={THEME.bg}/> : <PlayCircle size={32} fill={THEME.primary} color={THEME.bg}/>}</button>
                 <button onClick={proximoVideo} disabled={currentVideoIndex===playlist.length-1} style={{...btnStyle, background:'transparent', color:currentVideoIndex===playlist.length-1?THEME.cardBorder:THEME.text}}><SkipForward size={18}/></button>
               </div>
          ...
          <button onClick={iniciarRegistroRapido} style={{...btnStyle, width:'100%', padding:'24px', background: `linear-gradient(135deg, ${THEME.primary} 0%, ${THEME.primaryHover} 100%)`, color:'white', fontSize:'18px', boxShadow: `0 10px 20px -5px ${THEME.primary}66`, marginBottom:'24px'}}>
            <MousePointerClick size={28}/> MARCAR AÇÃO
          </button>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
            <button onClick={() => registrarFluxo('HAJIME')} style={{...btnStyle, background: THEME.success, color:'white', padding:'16px'}}><PlayCircle size={20}/> HAJIME</button>
            <button onClick={() => registrarFluxo('MATE')} style={{...btnStyle, background: THEME.danger, color:'white', padding:'16px'}}><PauseCircle size={20}/> MATE</button>
            <button onClick={() => registrarFluxo('GOLDEN SCORE')} style={{...btnStyle, background: THEME.warning, color:'#000', padding:'12px', fontSize:'13px'}}><Timer size={18}/> GOLDEN SCORE</button>
        ...
        <div style={{ flex: 2, width: '100%', display:'flex', flexDirection:'column', gap:'20px' }}>
              ...
              {playlist.map((item, index) => (
                <div key={index} onClick={() => selecionarVideo(index)} style={{ padding: '10px', cursor: 'pointer', background: index === currentVideoIndex ? `${THEME.primary}22` : 'transparent', borderRadius: '6px', marginBottom: '4px', display:'flex', justifyContent:'space-between', alignItems:'center', border: index===currentVideoIndex ? `1px solid ${THEME.primary}66` : '1px solid transparent' }}>
                  <div style={{display:'flex', alignItems:'center', gap:'8px', overflow:'hidden'}}>
                    {item.type === 'YOUTUBE' ? <Youtube size={14} color={THEME.danger}/> : <Film size={14} color={THEME.primary}/>}
          ...
          <div style={{ ...cardStyle, padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{fontSize: '10px', color: tempoDeLuta.isGS ? THEME.warning : THEME.textDim, fontWeight: '700'}}>{tempoDeLuta.isGS ? "GOLDEN SCORE" : "TEMPO"}</div>
              <div style={{fontSize: '32px', fontFamily: 'monospace', fontWeight: '700', color: tempoDeLuta.isGS ? THEME.warning : 'white'}}>{formatTime(tempoDeLuta.total)}</div>
            </div>
          ...
          <div style={{ ...cardStyle, padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{fontSize:'12px', color: THEME.textDim, fontWeight:'600', display:'flex', alignItems:'center', gap:'5px'}}><BarChart2 size={14}/> ANÁLISE EM TEMPO REAL</div>
            
            {/* 1. Volume de Ataque (Barra) */}
            <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
               <span style={{fontSize:'10px', width:'20px'}}>⚪</span>
               <div style={{flex:1, height:'8px', background: THEME.surface, borderRadius:'4px', overflow:'hidden'}}>
                 <div style={{width: `${(volumeAtaque.branco / (volumeAtaque.branco + volumeAtaque.azul || 1)) * 100}%`, height:'100%', background: 'white'}}></div>
               </div>
               <span style={{fontSize:'10px', width:'20px', textAlign:'right'}}>{volumeAtaque.branco}</span>
            </div>
            <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
               <span style={{fontSize:'10px', width:'20px'}}>🔵</span>
               <div style={{flex:1, height:'8px', background: THEME.surface, borderRadius:'4px', overflow:'hidden'}}>
                 <div style={{width: `${(volumeAtaque.azul / (volumeAtaque.branco + volumeAtaque.azul || 1)) * 100}%`, height:'100%', background: THEME.primary}}></div>
               </div>
               <span style={{fontSize:'10px', width:'20px', textAlign:'right'}}>{volumeAtaque.azul}</span>
            </div>

            {/* 2. Distribuição de Golpes (Donut Chart) */}
            {analyticsData.length > 0 && (
              <div style={{height: '150px', width:'100%', marginTop:'10px'}}>
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={analyticsData} innerRadius={40} outerRadius={60} paddingAngle={5} dataKey="value">
                      {analyticsData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                    <RechartsTooltip contentStyle={{background: THEME.card, border: `1px solid ${THEME.cardBorder}`, borderRadius:'8px'}} itemStyle={{color:THEME.text}}/>
                  </PieChart>
                </ResponsiveContainer>
                <div style={{textAlign:'center', fontSize:'10px', color:THEME.textDim}}>Distribuição Técnica</div>
              </div>
            )}
          </div>
